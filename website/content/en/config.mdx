...
Legacy `patch` / `mangle` examples in older projects should be migrated. New projects should use the modern shape directly for clearer typing and better tooling.
...
- `version` – Tailwind major version (`2`, `3`, or `4`).
- `v2` / `v3` – configure Tailwind v2/v3 instances living in workspace packages (`cwd`, `config`, `postcssPlugin`).
...
- `overwrite` – allow the patch operation to rewrite the target Tailwind files.
- `exposeContext` – expose Tailwind contexts at runtime so the API can read them back later.
- `extendLengthUnits` – customise the built-in length-unit patching; accepts `false` or an option object (`units`, `overwrite`, etc.).
...
- `driver` – pick `file` (default), `memory` for in-process caching, or `noop` to skip persistence when file writes are blocked.
...
### Recommended `sources` presets by integration type

Vite/Nuxt/Astro can usually keep broad defaults. For webpack-like pipelines, prefer narrower include rules to avoid unrelated resource transforms:
...
Use this as a starting point for Webpack and Vue CLI projects, then expand only when needed.
