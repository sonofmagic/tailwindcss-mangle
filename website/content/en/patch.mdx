...
Make sure your environment runs Node.js 22.18.x or 24.11 or newer.
...
`defineConfig` in v9 describes the modern `registry` shape only. Legacy aliases such as `registry.output` and `registry.tailwind` should be migrated before upgrading. The public option types also use the modern names only, such as `TailwindCssPatchOptions` and `TailwindCssOptions`.
