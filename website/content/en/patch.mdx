...
- Unified options object that normalises legacy `patch`/`cache` configs and unlocks cache, output, and feature flags in one place.
...
`defineConfig` still returns the legacy shape (`patch`, `applyPatches`, `tailwindcss`), but all new 8.0 fields are supported. You can migrate gradually by adding new keys; the runtime normalises them automatically.
...
- `CacheStore` – persistent cache manager that merges or overwrites class sets.
