...
<Callout type="info">
Use this page only for the shared toolchain layout change (`patch` / `mangle` -> `registry` / `transformer`). For the stricter `tailwindcss-patch` v9 alpha rules, follow the dedicated patch migration guide.
</Callout>
...
| `patch.output` | `registry.extract` | `filename` → `file`, `removeUniversalSelector` → `removeUniversalSelector`. |
| `patch.tailwindcss` | `registry.tailwindcss` | `package` → `packageName`, `v4` stays `v4`, `v3` stays `v3`, `v2` stays `v2`. |
| `patch.applyPatches` | `registry.apply` | `exportContext` becomes `exposeContext`, `extendLengthUnits` stays under `apply`. |
| `cache` / `overwrite` | `registry.cache` / `registry.apply.overwrite` | Cache options now live under the registry block; overwrite is nested under `apply`. |
...
`tw-patch install` and `tw-patch extract` automatically read the new structure. No flags change in v9, and the CLI still respects `--output`, `--format`, `--css`, and `--no-write`. If you rely on JSON editing in pipelines, adjust any scripts that wrote to `patch.output.filename` to look at `registry.extract.file` instead.
