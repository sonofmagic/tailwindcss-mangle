...
<Callout type="info">
这篇页面只覆盖共享工具链的结构迁移（`patch` / `mangle` -> `registry` / `transformer`）。如果你在升级更严格的 `tailwindcss-patch` v9 alpha，请优先看专门的 patch 迁移指南。
</Callout>
...
| `patch.output` | `registry.extract` | `filename` → `file`，`removeUniversalSelector` 保持为 `removeUniversalSelector`。 |
| `patch.tailwindcss` | `registry.tailwindcss` | `package` → `packageName`，`v4` / `v3` / `v2` 保持同名。 |
| `patch.applyPatches` | `registry.apply` | `exportContext` 改为 `exposeContext`，`extendLengthUnits` 仍在 `apply` 下。 |
| `cache` / `overwrite` | `registry.cache` / `registry.apply.overwrite` | 缓存相关配置统一放在 registry 内，overwrite 挪到 `apply`。 |
...
`tw-patch install` 与 `tw-patch extract` 会自动读取新结构，参数保持不变。若脚本中读取 `patch.output.filename`，记得改为 `registry.extract.file`。
