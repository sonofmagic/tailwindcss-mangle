...
旧版 `patch` / `mangle` 结构应该尽快迁移；新项目建议直接使用现代字段，类型提示和工具行为都会更清晰。
...
- `removeUniversalSelector` – 为 `true` 时从清单中移除 `*`。

### `tailwindcss`

- `version` – Tailwind 主版本号（`2` / `3` / `4`）。
- `v2` / `v3` – 配置 v2/v3 所在包的 `cwd`、`config`、`postcssPlugin`。
- `v4.cssEntries` – Tailwind v4 Oxide 扫描使用的 CSS 入口，可重复声明。
- `v4.sources` – v4 的内容源，默认为 `base` 下的 `**/*`。

### `apply`

- `overwrite` – 是否允许 patch 过程重写目标 Tailwind 文件。
- `exposeContext` – 将 Tailwind 运行时上下文导出，便于 API 回读。
- `extendLengthUnits` – 自定义长度单位补丁，支持 `false` 或配置对象（`units`、`overwrite` 等）。
...
- `driver` – 选择 `file`（默认）、`memory`（进程内缓存）或 `noop`（无法写文件时跳过持久化）。
...
Vite/Nuxt/Astro 通常可以沿用较宽松的默认扫描范围。对于 webpack 类流水线，建议先使用收敛规则，避免误处理无关资源：
...
这个配置适合作为 Webpack / Vue CLI 的起点，再按实际项目逐步放开范围。
