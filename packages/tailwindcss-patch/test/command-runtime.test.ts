...
describe('command runtime', () => {
  ...
  it('resolves cwd and supports override/non-override context loaders with caching', async () => {
    ...
    expect(loadPatchOptionsForWorkspaceMock).toHaveBeenCalledWith(path.resolve('./tmp/runtime-cwd'), undefined)
    expect(loadPatchOptionsForWorkspaceMock).toHaveBeenCalledWith(path.resolve('./tmp/runtime-cwd'), { override: true })
    expect(loadPatchOptionsForWorkspaceMock).toHaveBeenCalledWith(path.resolve('./tmp/runtime-cwd'), { force: true })
